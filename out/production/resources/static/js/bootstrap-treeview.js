...
// ***** If the current into the parent to child nodes not have any child node, then the [] is assigned to the parent node,
// ***** Node.nodes would otherwise undefined error occurs
	    if (node.nodes == null) {
	    	node.nodes = [];
	    }
	    if (options.node) {
	    ...
	    this.forEachIdentifier(identifiers, options, $.proxy(function (node, options) {
	        ...
	        if(parentNode && parentNode.nodes != null ){
	            ...
	            this.render();
	             ...
	             console.log ( 'the root node can not be deleted');
	        }
	    }, this));
	};
	/**
